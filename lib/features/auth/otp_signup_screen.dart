...
import 'password_signup_screen.dart';
import 'widgets/auth_inputs.dart';
import 'widgets/auth_scaffold.dart';

class OtpSignupScreen extends StatefulWidget {
  const OtpSignupScreen({super.key, required this.controller});
...
class _OtpSignupScreenState extends State<OtpSignupScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await auth.verifySignupOtp(phone: _form.fullPhone, code: _code);
      if (!mounted) return;
      await Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (_) => PasswordSignupScreen(controller: _form),
        ),
      );
    } catch (error) {
      if (!mounted) return;
  ...
  Future<void> _resend() async {
    ...
    try {
      final auth = AppScope.read(context).repositories.auth;
      await auth.resendSignupOtp(phone: _form.fullPhone);
      if (!mounted) return;
      _startCooldown();
  ...
  Widget build(BuildContext context) {
    return AuthScaffold(
      ...
      title: 'رمز التحقق',
      subtitle:
          'أدخل الرمز المكوّن من ٦ أرقام\nالذي أرسلناه إلى ${_form.fullPhone}',
      stepCount: 4,
      currentStep: 2,
      child: Column(
