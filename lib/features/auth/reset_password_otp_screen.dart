...
/// Reset-password step 2 — verify the OTP sent to [phone].
...
class _ResetPasswordOtpScreenState extends State<ResetPasswordOtpScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await auth.verifyPasswordResetOtp(phone: widget.phone, code: _code);
      if (!mounted) return;
  ...
  Future<void> _resend() async {
    ...
    try {
      final auth = AppScope.read(context).repositories.auth;
      await auth.sendPasswordResetOtp(phone: widget.phone);
      if (!mounted) return;
      _startCooldown();
